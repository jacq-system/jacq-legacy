...
/*
als Sicherung:
IP des Aufrufers in imgBrowser und showPic überprüfen und von 1. zum 2. senden
aktuelle zeit mitschicken und in showPic überprüfen
*/
    ...
    if ($ip==substr($param,0,8)) {
        if ($cw && $ch) {
            ...
            if( $height == 480 ) {
                ...
                if( $fmtime ) {
                    // Connect to the DB
                    $dbLinkPic = dbi_connect($_CONFIG['DATABASE']['PICTURES']);

                    // Check if the picture was updated
                    $result = $dbLinkPic->query( "SELECT ID FROM files WHERE file LIKE '$imgname' AND mtime > FROM_UNIXTIME( '$fmtime' )" );
