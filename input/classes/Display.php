...
class Display
{
    ...
    public static function taxonWithHybrids (int|string $taxonID, bool $withSeperator = false, bool $withID = false): string
    {
        try {
            ...
            if (count($rows) > 0) {
                ...
                return self::taxon($taxonID, $withSeperator, false, $withID);
            }
        }
        catch (Exception $e) {
            error_log("SEVERE SQL-ERROR IN CLASS. USER-ID = {$_SESSION['uid']}\n" . $e->__toString());
        ...
        try {
            ...
            if (!empty($row)) {
                if ($row['source'] == 'literature') {
                    return self::protolog($row['source_citationID'], true);
     ...
     */
    public static function sortItem(int|string $typ, int|string $id): string
    {
        if ($typ == $id) {
            return "&nbsp;&nbsp;v";
        } else if ($typ == -$id) {
