...
";
				...
				$search="'".dbi_escape_string("%".$params['mysqlSearch']) . "%'";
...
WHERE
 ...
 ";
		...
		$c1=0;
		$c2=0;$res=array();
		$resdb=dbi_query($sqlcountsearch);
		if($resdb){
			$row=mysqli_fetch_assoc($resdb);
			if(isset($row['c'])){
				$c1=$row['c'];
			}
		}

		$resdb=dbi_query($sqlcountall);
		if($resdb){
			$row=mysqli_fetch_assoc($resdb);
			if(isset($row['c'])){
				$c2=$row['c'];
