...
function make_taxon($value)
{
    ...
    if ($value && strlen($value) > 1) {
        ...
        if (!empty($pieces[1])) $sql .= "AND te.epithet LIKE '" . mysql_escape_string($pieces[1]) . "%' ";
...
function make_citation($value)
{
    ...
    //error_log("make_citation: result: " . var_export($results,true),0);
    return $results;
}
...
//error_log("editLitTaxSynonymy.php: BEGIN",0);
