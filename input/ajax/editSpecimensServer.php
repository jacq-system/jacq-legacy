...
function makeTaxonFromID($taxonID)
{
    $sql = "SELECT ts.taxonID, tg.genus,
            ...
            WHERE ts.taxonID = '" . intval($taxonID) . "'";
    $result = db_query($sql);
    if (mysql_num_rows($result) > 0) {
        $row = mysql_fetch_array($result);
        return taxon($row, false, false);
    } else {
        return '';
    }
}
        ...
        if ($foundIDs) {
            ...
            foreach ($foundIDs as $foundID => $v) {
                ...
                $ret .= "<li><a href=\"editSpecimens.php?sel=" . htmlentities("<$foundID>") . "&ptid=0\">"
                      . $row2['source_code'] . $row2['HerbNummer'] . ": " . makeTaxonFromID($row2['taxonID']) . "</a></li>";
            }
            $ret .= "</ul>";
