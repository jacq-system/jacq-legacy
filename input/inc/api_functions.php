...
function garbageCollection($id) {
  ...
  $result = db_query("SELECT specimen_ID FROM api.tbl_api_specimens WHERE specimen_ID='$id'");
  if (mysql_num_rows($result)==0) {
    db_query("DELETE FROM api.tbl_api_units WHERE specimenID='$id'");
    db_query("DELETE FROM api.tbl_api_units_identifications WHERE specimenID_fk='$id'");
  }
}
...
function update_tbl_api_units($id, $showError=true) {
  ...
  if ($row['series_number']) {
    if ($row['Nummer']) $CollectorNumber .= " ".$row['Nummer'];
    if ($row['alt_number'] && trim($row['alt_number'])!="s.n.") $CollectorNumber .= " ".$row['alt_number'];
    if ($row['series']) $CollectorNumber .= " ".$row['series'];
    $CollectorNumber .= " ".$row['series_number'];
  }
  else {
    if ($row['series']) $CollectorNumber .= " ".$row['series'];
    if ($row['Nummer']) $CollectorNumber .= " ".$row['Nummer'];
    if ($row['alt_number']) $CollectorNumber .= " ".$row['alt_number'];
    if (strstr($row['alt_number'],"s.n.")) $CollectorNumber .= " [".$row['Datum']."]";
  }
  ...
  $result = db_query("SELECT specimenID FROM api.tbl_api_units WHERE specimenID='$id'");
  if (mysql_num_rows($result)==0) {
  ...
  $sql .= "SET
           UnitID='".mysql_escape_string($UnitID)."',
           DateLastModified='".mysql_escape_string($row['aktualdatum'])."',
           Collectors='".mysql_escape_string($Collectors)."',
           CollectorNumber='".mysql_escape_string(trim($CollectorNumber))."',
           ...
           Locality='".mysql_escape_string($row['Fundort'])."',
...
function helper_tbl_api_units_identifications($id,$row,$det,$date,$identHistory,$Typestatus,$StoredUnderName,$TypifiedName) {
  ...
  $sql ="SELECT paginae, figures, ".
        ...
        "WHERE taxonID='".mysql_escape_string($row['taxonID'])."'";
  $result = db_query($sql);
  $row_citation = mysql_fetch_array($result);
  $citation = $row_citation['autor']." (".substr($row_citation['jahr'], 0, 4).")";
  if ($row_citation['suptitel']) $citation .= " in ".$row_citation['editor'].": ".$row_citation['suptitel'];
  if ($row_citation['periodicalID']) $citation .= " ".$row_citation['periodical'];
  $citation .= " ".$row_citation['vol'];
  if ($row_citation['part']) $citation .= " (".$row_citation['part'].")";
  ...
  $sql = "INSERT INTO api.tbl_api_units_identifications SET
           specimenID_fk='$id',
           Family='".mysql_escape_string($row['family'])."',
           Genus='".mysql_escape_string($row['genus'])."',
           Species='".mysql_escape_string($row['epithet'])."',
           Author='".mysql_escape_string($row['author'])."',
           Infra_specificRank='".mysql_escape_string($rank)."',
           Infra_specificEpithet='".mysql_escape_string($Infra_specificEpithet)."',
           Infra_specificAuthor='".mysql_escape_string($Infra_specificAuthor)."',
           Identifier='".mysql_escape_string($det)."',
           IdentificationDate='".mysql_escape_string($date)."',
           Typestatus='".mysql_escape_string($Typestatus)."',
           OtherScientificNames='".mysql_escape_string($identHistory)."',
           Protologue_citation='".mysql_escape_string($citation)."',
