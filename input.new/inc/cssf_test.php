...
class CSSF{
	...
	****************************************************************************/
	...
	public function label ($x, $y, $label, $link = "", $id = "")
    {
		...
		$xh = $x - $width;
		if ($width>0) {
            print "<div class=\"cssflabel w3-theme-l1\" ";
            if ($id) {
                print "id=\"$id\" ";
            }
	...
	public function buttonLink ($x, $y, $text, $link, $newwindow, $bgcol = "")
    {
		...
		print "><input tabindex=\"{$this->tabindex}\" class=\"cssfbutton w3-button w3-theme-l3\"";
		if ($bgcol) {
	...
	public function checkbox($x, $y, $name, $ischecked, $ignoretab = false)
    {
		...
		print "<input{$t} class=\"cssfcheckbox w3-check\" type=\"checkbox\" name=\"$name\"";
		if ($this->nameIsID) {
            print " id=\"$name\"";
        }
		if ($ischecked) {
    ...
    {
		$this->_divclass($x-0.2,$y+0.1,"cssfinput");
		print "<input class=\"cssfcheckbox\" type=\"checkbox w3-check\" name=\"$name\" onChange=\"$js\"";
		if ($this->nameIsID) {
            print " id=\"$name\"";
        }
		if ($ischecked) {
    ...
    {
		$this->_divclass($x,$y,"cssfinput");
		print "<select tabindex=\"{$this->tabindex}\" class=\"cssf w3-select\"";
		if ($bgcol) {
            print " style=\"background-color: $bgcol;\"";
        }
		if ($this->nameIsID) {
	...
	public function editDropdown($x, $y, $w, $name, $value, $options, $maxsize = 0, $jump = 0, $bgcol = "", $title = "", $js = "")
    {
		...
		print "<select class=\"cssf w3-select\"";
	...
	public function inputJqAutocomplete($x, $y, $w, $name, $value, $index, $serverScript, $maxsize = 0, $minLength = 1, $bgcol = "", $title = "",$autoFocus = false, $zeroOnEmpty = false, $class_attribute = "")
    {
		...
		print "<input  tabindex=\"{$this->tabindex}\" class='cssftextAutocomplete w3-input w3-border' style='width: {$w}em;";
		if ($bgcol) {
            print " background-color: $bgcol;";
        }
...
<?PHP
    ...
    {
		$this->_divclass($x,$y,"cssfinput");
		print "<input  tabindex=\"{$this->tabindex}\" class=\"cssftext w3-input w3-border\" style=\"width: " . $w . "em;";
		if ($bgcol) {
            print " background-color: $bgcol;";
        }
		print "\" type=\"text\" name=\"$name\" value=\"" . htmlspecialchars($value) . "\"";
