...
function showMatchJsonRPC($formData)
{
    ...
    try {
        if ($formData['database'] == 'vienna_common') {
            if ($showSynonyms) {
                $matches = $service->getMatchesCommonNamesWithSynonyms($searchtext);
            } else {
                $matches = $service->getMatchesCommonNames($searchtext);
            }
            if ($useNearMatch) {
                $matchesNearMatch = $service->getMatchesCommonNames($searchtext, true);
            } else {
                $matchesNearMatch = array();
            }
        } elseif ($formData['database'] == 'col') {
            $matches = $service->getMatchesCol($searchtext);
            if ($useNearMatch) {
                $matchesNearMatch = $service->getMatchesCol($searchtext, true);
            } else {
                $matchesNearMatch = array();
            }
        } elseif ($formData['database'] == 'fe') {
            $matches = $service->getMatchesFaunaEuropaea($searchtext);
            if ($useNearMatch) {
                $matchesNearMatch = $service->getMatchesFaunaEuropaea($searchtext, true);
            } else {
                $matchesNearMatch = array();
            }
        } else {
        ...
        $out = prettyPrintMatches($matches,$start,$stop,
        ...
        if (!empty($matches['error'])) {
                    ...
                    if ($matches['result'][$indexMatch]['type'] == 'uni') {
                        ...
                        if ($indexResult < $countResults) {
                            ...
                            foreach ($row['species'] as $key2 => $row2) {
                                $columnLeft[] = '<td>&nbsp;&nbsp;<b>' . $row2['taxon'] . ' <' . $row2['taxonID'] . '></b>'
            ...
            $out = "<a href='taxamatchExport.php?search=" . urlencode($searchtext) . "&db=" . $formData['database'] . "' target='_blank'>export csv</a><br>\n"
