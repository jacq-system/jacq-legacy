CREATE DEFINER=`root`@`localhost` FUNCTION `GetScientificNameString`(`p_taxonID` INT(11), `p_bAvoidHybridFormula` TINYINT(1) UNSIGNED, `p_bNoAuthor` TINYINT(1) UNSIGNED) RETURNS varchar(255) CHARSET utf8
    READS SQL DATA
BEGIN
  ...
  DECLARE v_ScientificNameString varchar(255) default NULL;
  ...
  THEN
    SET v_ScientificNameString = CONCAT( _buildScientificName( v_parent_1_ID, p_bNoAuthor ), " x ", _buildScientificName( v_parent_2_ID, p_bNoAuthor ) );
    
    -- Check if a third parent exists
    IF( v_parent_3_ID != 0 )
    THEN
    	SET v_ScientificNameString = CONCAT( v_ScientificNameString, " x ", _buildScientificName( v_parent_3_ID, p_bNoAuthor ) );
    END IF;

  -- Non-Hybrid
  ELSE
    SET v_ScientificNameString = _buildScientificName( p_taxonID, p_bNoAuthor );
  END IF;
